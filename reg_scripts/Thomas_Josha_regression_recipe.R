...
bank_loan_recipe_reg <-recipe(money_made_inv ~ annual_inc + avg_cur_bal + bc_util + delinq_2yrs + delinq_amnt + dti + int_rate + loan_amnt + mort_acc + num_sats + num_tl_120dpd_2m + num_tl_90g_dpd_24m + num_tl_30dpd + out_prncp_inv + pub_rec + tot_coll_amt + tot_cur_bal + total_rec_late_fee + sub_grade + term , data = bank_loan_train_reg) %>% 
